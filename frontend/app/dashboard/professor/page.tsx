  ...
  const handleDownloadSlip = async (id: number) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(e?.error || `Failed to generate advising slip (${res.status}).`);
        return;
      }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
                    ...
                    <div className="p-5">
                      ...
                      <div className="mt-3.5 flex items-center justify-between flex-wrap gap-2">
                        <div className="flex items-center gap-3">
                          ...
                          {/* Download system-generated advising slip */}
                          <button
                            onClick={() => handleDownloadSlip(c.id)}
                            disabled={downloadingSlip === c.id}
                            className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-white/5 text-gray-400 ring-1 ring-white/10 hover:bg-white/10 hover:text-gray-200 transition-colors disabled:opacity-50">
                          ...
                          {/* Download student's uploaded signed form */}
